...
- **Missing-key detection (blocking)** — When an API response omits a required column's key entirely (distinct from returning the key with value `null`), the extraction fails fast with `MissingKeyError` before any DataFrame is built. The check runs inside `parse_response()` (and `ServiceNowTableAsset._batch_to_df()`), so the first bad page aborts the run with a precise message — `"required column 'X' (API field 'path.to.x') is absent from response record #N"` — and the temp table is dropped. Columns listed in an asset's `optional_columns` attribute are exempted. The registry rejects `optional_columns` entries that are used in `primary_key` or any index, so identity/lookup fields cannot accidentally be opted out. This catches schema drift (API removed a field) that pandas would otherwise silently convert to `NaN`. An intermediate `null` in a dotted path (e.g., Jira's `fields.assignee: null` for unassigned issues) is treated as present-as-null, not missing — only an absent key fires the check.
- **Null-rate warnings (non-blocking)** — `Asset.validate_warnings()` emits a single consolidated warning per run when any non-PK column exceeds its null-rate threshold. The defaults are `default_null_threshold = 0.02` (2%) and per-column overrides via `column_null_thresholds = {"col": 0.5, ...}`; setting a column's threshold to `1.0` silences the warning for that column. The warning lands in `run_history.metadata.warnings` and is logged at `WARNING` level — promotion proceeds regardless. For hard-blocking absence detection, use `optional_columns` and let the missing-key check do its job; null rate and missing-key measure different failure modes.
