...
**Column validation:** `_batch_to_df()` raises a `ValueError` if any declared
column is missing from the API response. This catches schema mismatches early
(e.g., a field was deprecated or an ACL changed). If you see this error, either
remove the column from the asset definition or fix the ServiceNow field
permissions. Extra columns from the API that aren't declared are silently dropped.

**Type coercion:** pysnc returns all values as strings. `_batch_to_df()` auto-coerces
based on declared column types — `Boolean()` fields map `"true"`/`"false"` to
Python booleans, `Float()` fields are parsed via `pd.to_numeric`, and
