...
- **Request shape.** Each request POSTs `{"values": [...], "options": {"mode": "opaque", "format": "hex", "token_len": 12}}`. The `options` block tells the service how to shape its tokens; defaults match the standard configuration. Pass `options=...` to the `TokenizationClient` constructor for a different shape (e.g. `token_len=18`).
